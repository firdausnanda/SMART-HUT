...
export default function ApplicationLogo({ className = '', collapsed = false, ...props }) {
    ...
    const cdkName = user?.cdk?.nama || 'Administrator';
        ...
        <div className={`relative group flex items-center ${collapsed ? 'justify-center' : 'gap-3'} ${className}`} {...props}>
            ...
            {!collapsed && (
                <div className="flex flex-col min-w-0">
                    ...
                    <span className="text-[10px] font-bold text-primary-300 uppercase tracking-widest leading-none mt-1 truncate" title={cdkName}>
                        {cdkName}
                    </span>
                </div>
            )}
