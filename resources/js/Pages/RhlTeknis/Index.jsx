...
export default function Index({ auth, datas, stats, filters, availableYears, sumberDana }) {
  ...
  const handleImportSubmit = (e) => {
    ...
    router.post(route('rhl-teknis.preview-import'), { file: importFile }, {
        forceFormData: true,
        preserveScroll: true,
        onError: (errors) => {
            MySwal.fire({
  ...
  const handleSingleAction = (id, action) => {
    ...
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        ...
        router.post(route('rhl-teknis.single-workflow-action', id), data, {
          preserveScroll: true,
          onSuccess: () => {
            if (action === 'delete') {
              MySwal.fire({
          ...
          <div className="bg-gradient-to-r from-emerald-800 to-green-600 rounded-2xl p-8 text-white shadow-lg relative overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              <div className="flex gap-2">
                {canExport && (
                  <button onClick={() => window.location.href = route('rhl-teknis.export', { year: filters.year })} className="flex items-center gap-2 px-4 py-2.5 bg-primary-700 text-primary-100 rounded-xl font-bold text-sm shadow-sm hover:bg-primary-800 transition-colors border border-primary-600/50">
