...
export default function Edit({ auth, skemas, data: initialData }) {
  ...
  const submit = (e) => {
    e.preventDefault();
    put(route('skps.update', initialData.id));
  };
  ...
  return (
    ...
    >
      <Head title="Edit Data SK PS" />

      <div className="max-w-4xl mx-auto">
        <Link
          href={route('skps.index')}
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="p-8 border-b border-gray-100 bg-gray-50/50">
            ...
            <p className="mt-1 text-sm text-gray-500">
              Perbarui informasi di bawah ini untuk data SK Perhutanan Sosial.
            </p>
          </div>
          ...
          <div className="p-8">
            <form onSubmit={submit} className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                ...
                <div>
                  <InputLabel htmlFor="ps_area" value="Luas PS (Ha)" className="text-gray-700 font-bold mb-2" />
                  <div className="relative">
                    <TextInput
                      ...
                      value={data.ps_area}
                      onChange={(e) => setData('ps_area', e.target.value)}
                      required
                      placeholder="0.00"
                    />
                  ...
                  <InputError message={errors.ps_area} className="mt-2" />
                </div>

                <div>
                  <InputLabel htmlFor="number_of_kk" value="Jumlah KK" className="text-gray-700 font-bold mb-2" />
                  <div className="relative">
                    <TextInput
                      id="number_of_kk"
                      type="number"
                      className="w-full pr-12"
                      value={data.number_of_kk}
                      onChange={(e) => setData('number_of_kk', e.target.value)}
                      required
                      placeholder="0"
                    />
                    <div className="absolute inset-y-0 right-0 flex items-center pr-4 pointer-events-none text-gray-400 text-xs font-bold">KK</div>
                  </div>
                  <InputError message={errors.number_of_kk} className="mt-2" />
                </div>

                <div>
                  <InputLabel htmlFor="id_skema_perhutanan_sosial" value="Skema Perhutanan Sosial" className="text-gray-700 font-bold mb-2" />
                  <Select
                    options={skemas.map(s => ({ value: s.id, label: s.name }))}
                    onChange={(opt) => setData('id_skema_perhutanan_sosial', opt?.value || '')}
                ...
                <div>
                  <InputLabel htmlFor="potential" value="Potensi" className="text-gray-700 font-bold mb-2" />
                  <TextInput
                    id="potential"
                    type="text"
                    className="w-full"
                    value={data.potential}
                    onChange={(e) => setData('potential', e.target.value)}
                    required
                    placeholder="Masukkan Potensi"
                  />
                  <InputError message={errors.potential} className="mt-2" />
                </div>
              </div>
