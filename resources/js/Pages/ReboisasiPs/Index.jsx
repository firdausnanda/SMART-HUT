...
export default function Index({ auth, datas, stats, filters, availableYears, sumberDana }) {
  ...
  const handleBulkAction = (action) => {
    ...
    let title = '';
    const routeName = 'reboisasi-ps.bulk-workflow-action';
    let confirmText = '';
    let color = '#3085d6';
    let showInput = false;
  ...
  const handleImportSubmit = (e) => {
    ...
    router.post(route('reboisasi-ps.preview-import'), { file: importFile }, {
        forceFormData: true,
        preserveScroll: true,
        onError: (errors) => {
            MySwal.fire({
  ...
  const handleSingleAction = (id, action) => {
    ...
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        ...
        router.post(route('reboisasi-ps.single-workflow-action', id), data, {
          preserveScroll: true,
          onSuccess: () => {
            if (action === 'delete') {
              MySwal.fire({
                ...
                <tbody className="divide-y divide-gray-100">
                  {datas.data.map((item) => (
                    <tr key={item.id} className={`hover:bg-primary-50/30 transition-colors group ${selectedIds.includes(item.id) ? 'bg-primary-50/50' : ''}`}>
                      ...
                      <td className="px-6 py-4">
                        <div className="font-medium text-gray-800">{item.pengelola?.name ?? '-'}</div>
                        <div className="text-xs text-primary-600 font-bold uppercase tracking-tighter">
                          {item.village_rel?.name ?? '-'}, {item.district_rel?.name ?? '-'}, {item.regency_rel?.name ?? '-'}
                        </div>
                      </td>
