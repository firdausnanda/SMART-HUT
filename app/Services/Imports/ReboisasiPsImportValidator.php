...
class ReboisasiPsImportValidator
{
    /**
     * @param array $row
     * @param int $rowNumber
    ...
    public function __invoke(array $row, int $rowNumber)
    {
        ...
        if (empty($row['pengelola'])) {
            $errors[] = 'Pengelola harus diisi.';
        } else {
            $pengelolaExists = DB::table('m_pengelola_ps')
                ->whereRaw('LOWER(name) = ?', [strtolower(trim($row['pengelola']))])
                ->exists();
            if (!$pengelolaExists) {
                $errors[] = "Pengelola '{$row['pengelola']}' tidak ditemukan di Master Pengelola PS.";
            }
        }
